...
ppt_path = input('請輸入PPT檔案絕對路徑(默認路徑:'+default_path+'):')
# print(os.listdir(default_path))
print('\n'+default_path + '的PPT 檔案如下:')
print(ppt_files)
print(pptx_files)
ppt_filename = input('請輸入PPT檔案名字(e.g ‘詩歌.ppt’ 請輸入 ‘詩歌’):')
saveppt_ind = input('是否需要儲存PPT為JPG(Y/N) (默認Y):')
rename_ind = input('是否需要重命名(Y/N) (默認Y):')


def ppt2jpg():
    output_path = getFilePath(ppt_path, ppt_filename)
    if output_path == '':
...
def getFilePath(ppt_path, ppt_filename):
    if ppt_path != '':
        ...
        output_path = default_path + '\\' + ppt_filename
